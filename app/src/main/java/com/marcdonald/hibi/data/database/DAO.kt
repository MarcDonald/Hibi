...
interface DAO {
	...
	@Query("SELECT id, location, isFavourite, day, month, year, hour, minute, content FROM Entry as e INNER JOIN TagEntryRelation as ter ON e.id = ter.entryId WHERE ter.tagId = :tagId ORDER BY year DESC, month DESC, day DESC, hour DESC, minute DESC, id DESC")
	...
	@Query("SELECT * FROM NewWord ORDER BY id DESC")
	...
	@Query("SELECT id, location, isFavourite, day, month, year, hour, minute, content FROM Entry as e INNER JOIN BookEntryRelation as ber ON e.id = ber.entryId WHERE ber.bookId = :bookId ORDER BY year DESC, month DESC, day DESC, hour DESC, minute DESC, id DESC")
